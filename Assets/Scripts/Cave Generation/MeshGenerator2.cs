...
public class MeshGenerator2 : MonoBehaviour
{
    ...
    {
        return new Vector3(-width / 2 + .5f + position.x, 2, -height / 2 + .5f + position.y);
    }
    ...
    public void GenerateMesh(byte[,] configs, float squareSize)
    {
        ...
        for (int y = 0; y < configs.GetLength(0); y++)
        {
            ...
            {               
                marchingSquares[y, x] = new Square(CoordToWorldPoint(configs.GetLength(1), configs.GetLength(0),
                                                   new Vector2(x, y)), squareSize, configs[y,x]);
